...
def random_blurred(img, mask, filter_size=0, if_reverse=False, rate=0.1, num=1):
    ...
    if if_reverse:
        ...
        for i in range(b):
            ...
            for j in range(num):
                m = (mask_pool_sub == mask_pool_sub[mask_pool_sub >= threshhold][
                    np.random.randint(0, len(mask_pool_sub[mask_pool_sub >= threshhold]))])
                m2 = (mask_pool_sub2 == mask_pool_sub2[mask_pool_sub2 <= threshhold2][
                    np.random.randint(0, len(mask_pool_sub2[mask_pool_sub2 <= threshhold2]))])
                if len(m.nonzero()) > 1:
        ...
        if random.random() < 0.5:
            ...
            if random.random() < 0.5:
                result_1[i, :, np.max((0, x)): np.min((w - 1, w + x - 1)), np.max((0, y)):np.min((h - 1, h + y - 1))] \
                    = data_1[i, :, np.max((0, -x)):np.min((w - 1, w - 1 - x)),
                      np.max((0, -y)):np.min((h - 1, h - 1 - y))]
                result_2[i, :, :, :] = data_2[i, :, :, :]
            else:
                result_1[i, :, :, :] = data_1[i, :, :, :]
                result_2[i, :, np.max((0, x)):np.min((w - 1, w + x - 1)), np.max((0, y)):np.min((h - 1, h + y - 1))] \
                    = data_2[i, :, np.max((0, -x)):np.min((w - 1, w - 1 - x)),
                      np.max((0, -y)):np.min((h - 1, h - 1 - y))]
        else:
            result_1[i, :, :, :] = data_1[i, :, :, :]
            result_2[i, :, :, :] = data_2[i, :, :, :]
